...
class HMC(MonteCarlo):
  ...
  def build_update(self):
    ...
    if not self.transformations: # was initialized with auto_transform=False
      raise ValueError(
        "This implementation of HMC requires all latent variables"
        "to have unconstrained support. Please initialize with "
        "auto_transform=True (this is a no-op for variables whose "
        "support is already unconstrained).")
  ...
  def _log_joint_unconstrained(self, z_sample):
    """
    Given a sample in the unconstrained space, compute the log-joint
    mapping back into the constrained space. 
    """
    ...
    for z_unconstrained, qz_unconstrained in z_sample.items():
      ...
      try:
        ...
        log_det_jacobian += bij.inverse_log_det_jacobian(qz_unconstrained)
  ...
  def _log_joint(self, z_sample):
    ...
    # we are passed in unconstrained params, need to convert
    # back to constrained. what if this method just assumes it's passed z:constrained value?
